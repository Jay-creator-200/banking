...
export async function GET(req) {
  try {
    ...
    if (!['SUPER_ADMIN', 'MANAGER', 'ACCOUNTANT', 'AUDITOR', 'CASHIER'].includes(role)) {
      throw AppError.forbidden('You do not have permission to view Cash Flow reports');
    }
    ...
    const report = await financialReportService.getCashFlow({
      startDate,
      endDate,
      branchId,
    });
    ...
    await auditLogService.logAction(
      ...
      null,
      null,
      { reportType: 'CASH_FLOW', branchId, startDate, endDate }
    );
