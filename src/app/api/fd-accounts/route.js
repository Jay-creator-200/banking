...
export async function GET(req) {
  try {
    ...
    if (search) {
      ...
      filter.$or = [
        { fdAccountNo: { $regex: search, $options: 'i' } },
        { memberId: { $in: memberIds } },
      ];
    }

    const result = await fdService.findMany(filter, {
      page,
      limit,
      sort: '-startDate',
...
export async function POST(req) {
  try {
    ...
    if (!hasPermission(session, 'deposits.create')) {
      throw AppError.forbidden('You do not have permission to open FD accounts');
    }

    const body = await req.json();
    const account = await fdService.openAccount(body, session.user.id);

    return successResponse(account, 201, { message: 'FD account opened successfully' });
  } catch (error) {
    return errorResponse(error);
  }
}
